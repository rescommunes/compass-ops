...
The broader concept behind this project includes the ability to bridge development workflows with real production‑style environments. Future versions of this template may evolve in that direction over time.
...
flowchart LR
    User([User / Browser])
    compass-gate([compass-gate<br/>Login / Account UI])
    ...
    User -->|Login / Register / Account| compass-gate
    compass-gate -->|Self-service flows| Kratos
    compass-gate -->|Consent / tokens| Hydra
...
- [x] SOPS for securely managing your configuration secrets
...
- [x] **Kubernetes (Minikube, Cloud-ready)** — container orchestration and environment isolation
  ...
  - [x] Dev environment
    ...
    Supports local proxy-based development.
  ...
  - [ ] Hosting in a cloud environment
...
This extension manages the decryption of secrets when syncing with your charts.
...
These records ensure that all hostnames under dev.example.com resolve to your local machine.
...
```bash
minikube start -p compass --driver=docker --cpus=6 --memory=16384 
```
...
The mount options create a mounted path for persistent database or service storage. Adjust `"$HOME/dev-storage"` as you'd like in any normal file path (e.g., avoid cloud-synced folders).
...
> * [ ] To do: Explain how to find the values for your chosen DNS provider

The `values.example.yaml` file has examples of how to fill out your chosen DNS provider. 
...
The `secrets.example.yaml` file has examples of how to fill out your chosen DNS provider. 
...
Any time you change the chart `repositories` be sure to update the dependencies for Helm.
